...
*/
module dlirium.textfilter.markdown;
...
import dlirium.textfilter.syntax_d;
			...
			final switch(ln.type){
				...
				case LineType.Plain:
					if( lines.length >= 2 && lines[1].type == LineType.SetextHeader ){
						...
						b.headerLevel = setln.strip()[0] == '=' ? 2 : 3;
						lines.popFrontN(2);
					} else {
...
/+
 ...
 +/
Tuple!(string, string) getTagsForToken(string token)
{
	return tuple("<span class='highlighting " ~ getKeywordForToken_D(token) ~ "'>", "</span>");
}
...
private void writeBlock(R)(ref R dst, ref const Block block, LinkRef[string] links, MarkdownFlags flags)
{
	final switch(block.type){
		...
		case BlockType.Header:
			...
			dst.put(nstr);
			dst.put(" id=\""~ h);
			filterHtmlEscape(dst, block.text[0].toLower().removechars(" "));
			dst.put("\"><a href=\"#" ~ h);
			filterHtmlEscape(dst, block.text[0].toLower().removechars(" "));
			dst.put("\">");
		...
		switch( ln[0] ){
			...
			case '*':
				...
				if( auto em = parseEmphasis(ln, text) ){
					dst.put(std.array.replicate(c, em));
